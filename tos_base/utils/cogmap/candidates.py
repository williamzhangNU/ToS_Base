...
def calculate_other_candidates_metrics(env_data: Dict[str, Any], max_candidates: int = 3) -> Tuple[List[float], List[float]]:
    """Per-turn metrics for `other_candidates` vs GT `possible_positions`.

    Returns (other_candidates_f1_per_turn, other_candidates_count_per_turn).
    """
    ...
    def _score(pred: set[tuple[int, int]], gt_other: set[tuple[int, int]]) -> tuple[float, float]:
        """Return (f1, count_score) in [0,1]."""
        expected = min(int(max_candidates), len(gt_other))
        if expected == 0:
            return (1.0, 1.0) if len(pred) == 0 else (0.0, 0.0)
        ...
        return float(f1 * count_score), float(count_score)
    ...
    for log in turn_logs:
        ...
        if isinstance(possible_positions, dict):
            for gt_name, gt_pts in possible_positions.items():
                ...
                f1, cs = _score(pred_other, gt_other)
                scores.append(f1)
    ...
    return f1_per_turn, count_score_per_turn
